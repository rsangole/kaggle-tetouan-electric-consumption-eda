---
title: "Tetouan Electric Consumption - EDA & Predictive Modeling in R"
author: "R Sangole"
date: "Aug 4, 2022"
output: 
  html_document: 
...
dat %>% 
  timetk::plot_time_series_regression(
    datetime,
    zone1 ~ month.lbl + temperature + humidity + wind_speed + as.factor(day) + hour + week + lag(zone1),
    .show_summary = TRUE
  )
dat %>% 
  timetk::plot_time_series_regression(
    datetime,
    zone2 ~ month.lbl + temperature + humidity + wind_speed + as.factor(day) + hour + week + lag(zone2),
    .show_summary = TRUE
  )
```
...
# ```{r}
# dat %>% 
#   group_by(date = as.Date(dt_hr)) %>% 
#   summarise(mean_zone1 = mean(zone1)) %>% 
#   timetk::plot_stl_diagnostics(date, mean_zone1)
# ```
...
```{r message=FALSE, warning=FALSE}
dat %>% 
  select(temperature:wind_speed, month, day, hour, wday) %>%
...
biplots(pca, dat$month.lbl, "Month") -> p4
biplots(pca, cut(dat$zone1, 5), "Zone 1") -> p1
biplots(pca, cut(dat$zone2, 5), "Zone 2") -> p2
biplots(pca, cut(dat$zone3, 5),"Zone 3") -> p3
(p1|p2)/(p3|p4)
```
